...
- s6 镜像 [fcurrk/rustdesk-server-s6](https://hub.docker.com/r/fcurrk/rustdesk-server-s6)
 ...
 services:
   rustdesk:
     ports:
       ...
       - 21118:21118
       - 21119:21119
     image: fcurrk/rustdesk-server-s6:latest
     environment:
...
- 普通镜像 [fcurrk/rustdesk-server](https://hub.docker.com/r/fcurrk/rustdesk-server)
...
[**下载**](https://github.com/fcurrk/rustdesk-server/releases)
...
您可以在 [releases](https://github.com/fcurrk/rustdesk-server/releases) 页面中找到最新的服务端软件。
...
这个类型的镜像是基于 `ubuntu-20.04` 进行构建，镜像仅包含两个主要的可执行程序（`hbbr` 和 `hbbs`）。它们可以通过以下tag在 [Docker Hub](https://hub.docker.com/r/fcurrk/rustdesk-server/) 上获得：

| 架构      | image:tag                                 |
|---------| ----------------------------------------- |
| amd64   | `fcurrk/rustdesk-server:latest`         |
| arm64v8 | `fcurrk/rustdesk-server:latest-arm64v8` |

您可以使用以下命令，直接通过 ``docker run`` 來启动这些镜像：

```bash
docker run --name hbbs --net=host -v "$PWD/data:/root" -d fcurrk/rustdesk-server:latest hbbs -r <relay-server-ip[:port]> 
docker run --name hbbr --net=host -v "$PWD/data:/root" -d fcurrk/rustdesk-server:latest hbbr 
```
...
```bash
docker run --name hbbs -p 21115:21115 -p 21116:21116 -p 21116:21116/udp -p 21118:21118 -v "$PWD/data:/root" -d fcurrk/rustdesk-server:latest hbbs -r <relay-server-ip[:port]> 
docker run --name hbbr -p 21117:21117 -p 21119:21119 -v "$PWD/data:/root" -d fcurrk/rustdesk-server:latest hbbr 
```
...
services:
  hbbs:
    ...
    ports:
      ...
      - 21116:21116/udp
      - 21118:21118
    image: fcurrk/rustdesk-server:latest
...
> 这些镜像是针对 `busybox:stable` 构建的，并添加了可执行程序（hbbr 和 hbbs）以及 [S6-overlay](https://github.com/just-containers/s6-overlay)。 它们可以使用以下tag在 [Docker hub](https://hub.docker.com/r/fcurrk/rustdesk-server-s6/) 上获取：


| 架構      | version | image:tag                                    |
| --------- | ------- | -------------------------------------------- |
| multiarch | latest  | `fcurrk/rustdesk-server-s6:latest`         |
| amd64     | latest  | `fcurrk/rustdesk-server-s6:latest-amd64`   |
| i386      | latest  | `fcurrk/rustdesk-server-s6:latest-i386`    |
| arm64v8   | latest  | `fcurrk/rustdesk-server-s6:latest-arm64v8` |
| armv7     | latest  | `fcurrk/rustdesk-server-s6:latest-armv7`   |
| multiarch | 2       | `fcurrk/rustdesk-server-s6:2`              |
| amd64     | 2       | `fcurrk/rustdesk-server-s6:2-amd64`        |
| i386      | 2       | `fcurrk/rustdesk-server-s6:2-i386`         |
| arm64v8   | 2       | `fcurrk/rustdesk-server-s6:2-arm64v8`      |
| armv7     | 2       | `fcurrk/rustdesk-server-s6:2-armv7`        |
| multiarch | 2.0.0   | `fcurrk/rustdesk-server-s6:2.0.0`          |
| amd64     | 2.0.0   | `fcurrk/rustdesk-server-s6:2.0.0-amd64`    |
| i386      | 2.0.0   | `fcurrk/rustdesk-server-s6:2.0.0-i386`     |
| arm64v8   | 2.0.0   | `fcurrk/rustdesk-server-s6:2.0.0-arm64v8`  |
| armv7     | 2.0.0   | `fcurrk/rustdesk-server-s6:2.0.0-armv7`    |
...
docker run --name rustdesk-server \ 
  ...
  -e "ENCRYPTED_ONLY=1" \
  -v "$PWD/data:/data" -d fcurrk/rustdesk-server-s6:latest
```
...
docker run --name rustdesk-server \ 
  ...
  -v "$PWD/db:/db" -d fcurrk/rustdesk-server-s6:latest
```
...
docker service create --name rustdesk-server \
  ...
  fcurrk/rustdesk-server-s6:latest
```
...
```bash
docker run --rm --entrypoint /usr/bin/rustdesk-utils  fcurrk/rustdesk-server-s6:latest genkeypair
```
...
每个可执行文件都有单独的 .deb 套件可供使用，您可以在 [releases](https://github.com/fcurrk/rustdesk-server/releases) 页面中找到它們。
這些套件适用于以下发行版：
